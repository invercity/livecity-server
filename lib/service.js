...
// delta for comparing
const __deltaDistance = config.get('app:deltaDistance');
// speed - 50 km/h, will be replaced later
const __defaultSpeed = config.get('app:defaultSpeed')/__minutePerHour;
// default period for bus stop (minutes)
const __defaultStopTime = config.get('app:defaultStopTime');
// default radius for searching guide points
const __defaultSearchPointRadius = config.get('app:defaultSearchPointRadius');
                ...
                __this.__route.findById(routeId, function(err, route){
                    ...
                    if ((err) || (!route)) {
                       ...
                       __this.__trans.find({route: routeId}, function(err, allTransport) {
                            ...
                            const nearTransport = null;
                            // link to the near transport on a lap+
                            const nearTransportLap = null;
                            ...
                            async.eachSeries(allTransport, function(transport, callback) {
                                ...
                                if ((nearTransport) || (nearTransportLap)) {
                                    ...
                                    if (fromPointPosition <= currentPointPosition) {
                                        ...
                                        for (const x = fromPointPosition; x < currentPointPosition; x++) {
                                            nodes++;
                                    ...
                                    // if this trans on a circle to current station
                                        ...
                                        for (const y = 0; y < currentPointPosition - 1; y++) {
                                            nodes++;
                                            distance += route.nodes[y].total;
                                        }
                                        // sum of nodes pos..nodes.length
                                        for (const z = fromPointPosition; z < route.nodes.length; z++) {
                                            nodes++;
                                ...
                                _this.__route.findById(routeId, function(err, currentRoute){
                                    ...
                                    if (!err) {
                                        ...
                                        async.filterSeries(endResult.ids, function(endPoint, callback){
                                            ...
                                            if (xRoutes.length > 0) {
                                                for (const i=0;i<xRoutes.length;i++) {
                                                    ...
                                                    if ((indexA !== -1) && (indexB !== -1)) {
                                                        ...
                                                        if (indexA < indexB) {
                                                            ...
                                                            for (const indx=indexA;indx<indexB;indx++)
                                                                distance+= currentRoute.nodes[indx].total;
                                                        }
                                                        // if A point AFTER B point
                                                        else {
                                                            for (const i=indexB;i<currentRoute.nodes.length;i++)
                                                                distance+= currentRoute.nodes[i].total;
                                                            for (const i=0;i<indexB;i++)
                                                                distance+= currentRoute.nodes[i].total;
                                                        }
                                                        // calculate WALK TO
                           ...
                           if (left < __deltaDistance) {
                               ...
                               async.detect(currentRoute.points, function(point, callback) {
                                   ...
                                   if (nextPoint) {
                                       ...
                                       const index = currentRoute.points.indexOf(nextPoint) + 1;
                           ...
                           if (!err) {
                               ...
                               async.detect(points, function(point, callback) {
                                   ...
                                   if (currentPoint) {
                                       ...
                                       const index = currentRoute.points.indexOf(currentPoint._id) + 1;
